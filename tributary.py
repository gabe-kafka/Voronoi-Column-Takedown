...
# Extract WALL layer entities from input
wall_entities = [e for e in input_msp if hasattr(e.dxf, 'layer') and e.dxf.layer == 'WALL']
...
for wall_idx, entity in enumerate(wall_entities):
    try:
        ...
        if entity.dxftype() == 'LWPOLYLINE':
            ...
            if len(points) >= 2:
                # Convert from inches to feet (matching the coordinate system)
        ...
        # Generate support points along the wall every 4 inches for dense sampling
        # This approximates continuous wall behavior where walls act as infinite support lines
        if wall_line and wall_line.length > 0:
            support_points = []
            spacing = 0.333  # 4 inches = 0.333 feet (dense sampling for continuous wall approximation)
            distance = 0.0
            total_length = wall_line.length
            
            while distance <= total_length:
                # Generate point at current distance along the wall
                support_point = wall_line.interpolate(distance)
                support_points.append(Point(support_point.x, support_point.y))
                distance += spacing
            
            # Store wall data including entity reference, support points, and indices
            wall_data = {
                'wall_index': wall_idx,
                'entity': entity,
                'wall_line': wall_line,
                'support_points': support_points,
                'support_point_indices': [],  # Will be filled when combining with columns
                'tributary_regions': [],  # Will be filled after tributary calculation
                'merged_region': None,  # Will be filled after merging
                'total_area': 0.0  # Will be calculated after merging
            }
...
# Check if boundary_id column exists in CSV
if 'boundary_id' in boundary_df.columns:
    # Group boundary data by boundary_id to create separate floor plan polygons
    unique_boundary_ids = boundary_df['boundary_id'].unique()
    
    for idx, boundary_id in enumerate(unique_boundary_ids):
        # Get all boundary data for this floor plan
        floor_boundary_df = boundary_df[boundary_df['boundary_id'] == boundary_id]
        
        # Handle different boundary types
        boundary_lines = []
        slab_polygon = None
        
        # Group LWPOLYLINE vertices
        lwpolylines = floor_boundary_df[floor_boundary_df['type'] == 'LWPOLYLINE'].copy()
        if not lwpolylines.empty:
            # Sort by vertex_index to ensure proper order
            lwpolylines = lwpolylines.sort_values('vertex_index')
            vertices = [(row['x'], row['y']) for _, row in lwpolylines.iterrows()]
            # Create a closed polygon from vertices
            if len(vertices) >= 3:
                vertices.append(vertices[0])  # Close the polygon
                slab_polygon = Polygon(vertices).buffer(0.006)
        else:
            # Handle LINE entities
            lines = floor_boundary_df[floor_boundary_df['type'] == 'LINE']
            boundary_lines = [
                LineString([(row['start_x'], row['start_y']), (row['end_x'], row['end_y'])])
                for _, row in lines.iterrows() if pd.notna(row['start_x'])
            ]
            if boundary_lines:
                slab_polygon = list(polygonize(unary_union(boundary_lines)))[0].buffer(0.006)
        
        # Store each floor plan as a dictionary
        if slab_polygon and not slab_polygon.is_empty:
            floor_plan = {
                'index': idx,
    ...
    # If no boundary_id exists, treat all boundaries as a single floor plan (FLOOR_0)
    boundary_lines = []
    slab_polygon = None
    
    # Group LWPOLYLINE vertices
    lwpolylines = boundary_df[boundary_df['type'] == 'LWPOLYLINE'].copy()
    if not lwpolylines.empty:
        # Sort by vertex_index to ensure proper order
        lwpolylines = lwpolylines.sort_values('vertex_index')
        vertices = [(row['x'], row['y']) for _, row in lwpolylines.iterrows()]
        ...
        # Handle LINE entities
        lines = boundary_df[boundary_df['type'] == 'LINE']
        boundary_lines = [
            LineString([(row['start_x'], row['start_y']), (row['end_x'], row['end_y'])])
            for _, row in lines.iterrows() if pd.notna(row['start_x'])
        ]
    ...
    if slab_polygon and not slab_polygon.is_empty:
        ...
        raise ValueError("No valid boundary data found")
...
# Associate floor numbers with floor plans based on proximity (Y-coordinate matching)
# Assign the same floor number to ALL boundaries within a vertical tolerance
if floor_numbers_list:
    vertical_tolerance = 75.0  # Assign same floor number to boundaries within 75 feet vertically
    
    for floor_num_dict in floor_numbers_list:
        floor_text = floor_num_dict['floor_number']
        floor_x = floor_num_dict['x']
        floor_y = floor_num_dict['y']
        
        # Find ALL floor plans within vertical tolerance
        assigned_count = 0
        for floor_plan in floor_plans:
            # Get the centroid Y-coordinate of the floor plan
            centroid_y = floor_plan['slab_polygon'].centroid.y
            
            # Calculate vertical distance
            distance = abs(centroid_y - floor_y)
            
            # Assign floor number if within tolerance
            if distance < vertical_tolerance:
                floor_plan['floor_number'] = floor_text
                assigned_count += 1
        
        if assigned_count > 0:
            print(f"  Assigned '{floor_text}' to {assigned_count} floor plan(s) near y≈{floor_y:.1f}")

# Assign default floor numbers to any floor plans without assigned floor numbers
for floor_plan in floor_plans:
    if 'floor_number' not in floor_plan or floor_plan.get('floor_number') is None:
        floor_plan['floor_number'] = floor_plan['boundary_id']
...
# --- Assign wall entities to floor plans based on spatial containment ---
orphaned_walls = []

for wall_data in wall_data_list:
    # Check which floor plan contains this wall (using centroid or endpoints)
    wall_line = wall_data['wall_line']
    wall_centroid = wall_line.centroid
    
    assigned = False
    for floor_plan in floor_plans:
        # Check if centroid is contained in the floor plan
        if floor_plan['slab_polygon'].contains(wall_centroid):
            # Store wall data in the corresponding floor plan dictionary
            floor_plan['walls'].append(wall_data)
            assigned = True
            break
    
    # Track walls that don't belong to any floor plan
    if not assigned:
...
for col_point, original_idx in zip(all_column_points, all_column_original_indices):
    ...
    for floor_plan in floor_plans:
        # Use Shapely's contains() method to check spatial containment
        if floor_plan['slab_polygon'].contains(col_point):
            # Store assigned columns and their original indices
            floor_plan['column_points'].append(col_point)
            floor_plan['column_indices'].append(original_idx)
            assigned = True
            break  # Column can only belong to one floor plan
    
    # Track orphaned columns
    if not assigned:
...
for floor_plan in floor_plans:
    ...
    if not column_labels or not column_points:
        ...
        floor_plan['column_labels'] = [f"UNLABELED_{idx}" for idx in range(len(column_points))]
        continue
    ...
    for point_idx in range(len(column_points)):
        if point_idx in floor_plan['label_associations']:
            ...
            floor_plan['column_labels'].append(f"UNLABELED_{point_idx}")
...
for floor_plan in floor_plans:
    ...
    print(f"\n--- Floor {floor_idx} ({floor_plan['boundary_id']}) ---")
    print(f"Columns: {len(column_points)}")
    print(f"Walls: {len(walls)}")
    
    # Combine column points and wall support points into a single list for tributary calculation
    combined_points = []
    point_metadata = []  # Track which points are columns vs wall supports
    
    # Add column points first
    column_labels_list = floor_plan.get('column_labels', [])
    for col_idx, col_point in enumerate(column_points):
        combined_points.append(col_point)
        # Get column label from the column_labels list (parallel to column_points)
        column_label = column_labels_list[col_idx] if col_idx < len(column_labels_list) else f"UNLABELED_{col_idx}"
    ...
    # Add wall support points
    wall_support_start_idx = len(combined_points)
    for wall_data in walls:
        wall_idx = wall_data['wall_index']
        for support_point in wall_data['support_points']:
            point_idx = len(combined_points)
            combined_points.append(support_point)
            point_metadata.append({
                'type': 'wall',
                'column_index': None,
                'wall_index': wall_idx,
                'wall_data': wall_data,
                'column_label': None  # Wall support points have no column label
            })
            # Store the index in the combined list for later grouping
            wall_data['support_point_indices'].append(point_idx)
        
    total_support_points = sum(len(w['support_points']) for w in walls)
    print(f"Total points for tributary calculation: {len(combined_points)} ({len(column_points)} columns + {total_support_points} wall supports)")
    
    # Skip if no points in this floor plan
    if len(combined_points) == 0:
        print("No points found - skipping tributary calculation")
        floor_plan['regions'] = []
        floor_plan['areas'] = []
        floor_plan['point_metadata'] = []
        continue
    
    # Calculate bounding box for this floor plan
    minx, miny, maxx, maxy = slab_polygon.bounds
    bbox = {'minx': minx - 5, 'maxx': maxx + 5, 'miny': miny - 5, 'maxy': maxy + 5}
    
    # Run the existing Voronoi-like tributary calculation on the combined point set
    regions = []
    for i, P in enumerate(combined_points):
        region = slab_polygon
        for j, Q in enumerate(combined_points):
            if i == j: continue
    ...
    # Store tributary regions with metadata indicating if they belong to a column or wall
    for i, metadata in enumerate(point_metadata):
        if metadata['type'] == 'wall':
            # Store tributary region in the wall data structure
            wall_data = metadata['wall_data']
            wall_data['tributary_regions'].append(regions[i])
    
    # Store calculated regions and areas in the floor plan dictionary
    floor_plan['regions'] = regions
    floor_plan['areas'] = areas
    floor_plan['point_metadata'] = point_metadata
    
    # Find and report large areas above threshold (columns only for now)
    large_area_idxs = [i for i, (area, meta) in enumerate(zip(areas, point_metadata)) 
                       if area > threshold and meta['type'] == 'column']
    large_area_idxs_sorted = sorted(large_area_idxs, key=lambda i: areas[i], reverse=True)
    
    print(f"Tributary regions calculated: {len(regions)}")
    print(f"Large column tributary areas (>{threshold} SF): {len(large_area_idxs)}")
    
    if large_area_idxs_sorted:
        print("Large column tributary areas (sorted by size):")
        for rank, idx in enumerate(large_area_idxs_sorted, 1):
            col_point = combined_points[idx]
            print(f"  {rank}. Column {idx} at ({col_point.x:.2f}, {col_point.y:.2f}): {math.ceil(areas[idx]):.0f} SF")
    
    # --- Group and merge wall tributary regions ---
    print("\n--- Wall Tributary Regions ---")
    for wall_data in walls:
        wall_idx = wall_data['wall_index']
        tributary_regions = wall_data['tributary_regions']
        
        if not tributary_regions:
            print(f"Wall {wall_idx}: No tributary regions")
            continue
        
        # Filter out empty regions
        valid_regions = [r for r in tributary_regions if not r.is_empty]
        
        if not valid_regions:
            print(f"Wall {wall_idx}: No valid tributary regions")
            continue
        
        # Use Shapely's unary_union to merge all tributary polygons belonging to the same wall
        try:
            merged_region = unary_union(valid_regions)
            
            # Calculate total area for each merged wall tributary region
            total_area = merged_region.area
            
            # Store merged regions in wall data structure
            wall_data['merged_region'] = merged_region
            wall_data['total_area'] = total_area
            
            print(f"Wall {wall_idx}: {len(valid_regions)} regions merged, Total area: {math.ceil(total_area):.0f} SF")
            
        except Exception as e:
            print(f"Warning: Could not merge tributary regions for wall {wall_idx}: {e}")
...
# Scale factor to convert from feet (internal calculations) to inches (DXF units)
FEET_TO_INCHES = 12
...
for floor_plan in floor_plans:
    ...
    # Extract exterior coordinates from the slab_polygon Shapely object
    slab_coords = list(slab_polygon.exterior.coords)
    # Scale coordinates from feet to inches for DXF output
    slab_coords_inches = [(x * FEET_TO_INCHES, y * FEET_TO_INCHES) for x, y in slab_coords]
    # Create closed LWPOLYLINE entity on FLOOR_{f}_BOUNDARY layer
    slab_polyline = msp.add_lwpolyline(slab_coords_inches, close=True)
    ...
    for col_point in column_points:
        ...
        point_entity = msp.add_point((col_point.x * FEET_TO_INCHES, col_point.y * FEET_TO_INCHES))
    ...
    point_metadata = floor_plan['point_metadata']
    for i, (region, metadata) in enumerate(zip(regions, point_metadata)):
        # Only output column tributary regions here (walls are handled separately)
        ...
        # Scale coordinates from feet to inches for DXF output
        region_coords_inches = [(x * FEET_TO_INCHES, y * FEET_TO_INCHES) for x, y in region_coords]
        
        # Create layer name for this tributary region using FLOOR_{f}_TRIBUTARY_COL_{i} pattern
        col_idx = metadata['column_index']
    ...
    for wall_data in walls:
        ...
        for poly in polygons_to_output:
            ...
            # Scale coordinates from feet to inches for DXF output
            wall_coords_inches = [(x * FEET_TO_INCHES, y * FEET_TO_INCHES) for x, y in wall_coords]
        ...
        wall_text_entity.dxf.height = 2.0 * FEET_TO_INCHES  # Scale text height to inches
        wall_text_entity.dxf.insert = (wall_centroid.x * FEET_TO_INCHES, wall_centroid.y * FEET_TO_INCHES)
        wall_text_entity.dxf.halign = 1  # center horizontal alignment
        wall_text_entity.dxf.valign = 2  # middle vertical alignment
        wall_text_entity.dxf.align_point = (wall_centroid.x * FEET_TO_INCHES, wall_centroid.y * FEET_TO_INCHES)
    ...
    for i, (region, metadata) in enumerate(zip(regions, point_metadata)):
        # Only output column area labels here (wall areas are handled separately)
        if metadata['type'] != 'column':
            continue
        ...
        area_text = f"{math.ceil(areas[i]):.0f} SF"
        ...
        text_entity.dxf.height = 2.0 * FEET_TO_INCHES  # Scale text height to inches
        text_entity.dxf.insert = (centroid.x * FEET_TO_INCHES, centroid.y * FEET_TO_INCHES)
        ...
        text_entity.dxf.align_point = (centroid.x * FEET_TO_INCHES, centroid.y * FEET_TO_INCHES)
    ...
    if fascade_data and fascade_data.get('segments'):
        ...
        for segment in fascade_data.get('segments', []):
            ...
            coords_inches = [(x * FEET_TO_INCHES, y * FEET_TO_INCHES) for x, y in coords]
            ...
            try:
                ...
                text_point = (offset_x * FEET_TO_INCHES, offset_y * FEET_TO_INCHES)
                length_text = f"{seg_length_ft:.0f} FT"

                length_entity = msp.add_text(length_text)
                length_entity.dxf.layer = segment_poly.dxf.layer
                length_entity.dxf.height = 1.0 * FEET_TO_INCHES  # half the area text height
...
for floor_plan in floor_plans:
    ...
    large_column_areas = [areas[i] for i, meta in enumerate(point_metadata) 
                          if meta['type'] == 'column' and areas[i] > threshold]
