...
def collect_master_matrix_data(floor_plans):
    ...
    for floor_plan in floor_plans:
        ...
        # Iterate through point_metadata to extract column tributary areas
        for i, metadata in enumerate(point_metadata):
            # Only process column points (skip wall support points)
            ...
            # Get tributary area and round up to nearest foot
        ...
        for floor_id in fascade_floor_numbers:
            ...
            for label in fascade_labels:
                ...
                if length_value is None:
                    ...
                    rounded_value = int(round(length_value))
                    if rounded_value > 0:
                        row.append(rounded_value)
                    else:
                        row.append("")
            # Append totals/perimeter columns
            rounded_total = int(round(fascade_assigned_totals.get(floor_id, 0.0)))
            row.append(rounded_total)
            perimeter_value = int(round(fascade_perimeters.get(floor_id, 0.0)))
